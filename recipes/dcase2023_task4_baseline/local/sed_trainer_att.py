...
from desed_task.data_augm import mixup, frame_shift, feature_transformation
...
class SEDTask4_att(pl.LightningModule):
    ...
    def __init__(
        ...
        super(SEDTask4_att, self).__init__()
        ...
        self.obj_metric = self.hparams["training"].get("obj_metric_synth_type")
    ...
    def training_step(self, batch, batch_indx):
        ...
        #Frame shift
        if self.hparams["training"]["time_shift"] and self.hparams["training"]["time_shift_prob"] > random.random():
            features, labels = frame_shift(features, labels, self.hparams["data"]["net_subsample"])
        ...
        if self.hparams["training"]["transform"]["n_transform"] != 0:
            features = feature_transformation(features, **self.hparams["training"]["transform"])
        ...
        tot_loss_supervised = loss_strong + self.hparams["training"]["w_weak"]*loss_weak
        ...
        strong_self_sup_loss = self.selfsup_loss(
            torch.sigmoid(strong_preds_student), torch.sigmoid(strong_preds_teacher.detach())
        )
        weak_self_sup_loss = self.selfsup_loss(
            torch.sigmoid(weak_preds_student), torch.sigmoid(weak_preds_teacher.detach())
        )
    ...
    def validation_epoch_end(self, outputs):
        ...
        # Select best model based on teacher or student
        obj_metric_synth_type = self.obj_metric
        obj_metric_weak = self.hparams["training"].get("obj_metric_weak")
        model_selection = self.hparams["training"].get("model_selection")
        if model_selection == "student":
            if obj_metric_synth_type is None:
                ...
                synth_metric = psds2_student_sed_scores_eval
            else:
                raise NotImplementedError(
        ...
        if obj_metric_weak:
            obj_metric = torch.tensor(weak_metric + synth_metric)
        else:
